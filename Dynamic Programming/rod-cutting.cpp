...
int helper(vector<int>&price , int n , int ind , vector<vector<int>>&dp){
	if(ind == 0){
		return n*price[0];
	}
	if(dp[ind][n] != -1){
	...
	int not_taken  = 0 + helper(price , n ,ind-1 ,dp);
	int taken  = -1e9;
	int rodlength  = ind+1;
	if(rodlength <= n){
		taken = price[ind] + helper(price , n-rodlength , ind ,dp);
	}
	
	return dp[ind][n] = max(taken , not_taken);
}
  ...
  for(int ind = 1 ; ind<n;ind++){
    for(int length = 0;length<=n;length++){
      ...
      if(rodlength <= length){
		...
		dp[ind][length] = max(taken , not_taken);
    }
  }
  return dp[n-1][n];
	// Write your code here.
