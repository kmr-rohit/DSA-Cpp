...
// Need to make a comparator for sorting edges wrt weight
bool comp(node a , node b){
  return a.wt<b.wt;
}

// For finding parent node in disjoint set 
int findPar(int u , vector<int> &parent){
  if(u == parent[u])return u;
  return parent[u] = findPar(parent[u] , parent);
}

// Union making void function 

void unionn(int u , int v , vector<int> &parent , vector<int> &rank){
  u = findPar(u , parent);
  v = findPar(v , parent);
  // cheack whether they have same parent or diff
  if(rank[u] < rank[v]){
    parent[u] = v;
  }
...
int main(){
  ...
  // Setting parent and rank vectors 
  vector<int> parent(N);
  for(int i = 0;i<N;i++){
    parent[i] = i;
  }
